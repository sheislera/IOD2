...
def seed_data():
    ...
    with sqlite3.connect("voting.db") as conn:
        ...
        for expert in experts:
            cursor.execute("INSERT OR IGNORE INTO experts (name) VALUES (?)", (expert,))
        conn.commit()
...
def apply_heuristics(cursor):
    """Застосовує всі евристики та повертає результати фільтрації"""
    results = []
    ...
    cursor.execute("""
        ...
        HAVING COUNT(*) = 1
    """)
    e1_performers = {row[0] for row in cursor.fetchall()}
    excluded_e1 = current_set - e1_performers
    results.append(("E1: 1 раз на 3 місці", [all_performers[pid] for pid in excluded_e1]))
    current_set = current_set - excluded_e1
    ...
    cursor.execute("""
        ...
        HAVING COUNT(*) = 1
    """)
    e2_performers = {row[0] for row in cursor.fetchall()}
    excluded_e2 = current_set - e2_performers
    results.append(("E2: 1 раз на 2 місці", [all_performers[pid] for pid in excluded_e2]))
    current_set = current_set - excluded_e2
    ...
    cursor.execute("""
        ...
        HAVING COUNT(*) = 1
    """)
    e3_performers = {row[0] for row in cursor.fetchall()}
    excluded_e3 = current_set - e3_performers
    results.append(("E3: 1 раз на 1 місці", [all_performers[pid] for pid in excluded_e3]))
    current_set = current_set - excluded_e3
    ...
    cursor.execute("""
        ...
        HAVING COUNT(*) = 2
    """)
    e4_performers = {row[0] for row in cursor.fetchall()}
    excluded_e4 = current_set - e4_performers
    results.append(("E4: 2 рази на 3 місці", [all_performers[pid] for pid in excluded_e4]))
    current_set = current_set - excluded_e4
    ...
    """)
    e5_performers = {row[0] for row in cursor.fetchall()}
    excluded_e5 = current_set - e5_performers
    results.append(("E5: 1 раз на 3 і 1 раз на 2 місці", [all_performers[pid] for pid in excluded_e5]))
    current_set = current_set - excluded_e5
    ...
    """)
    e6_performers = {row[0] for row in cursor.fetchall()}
    excluded_e6 = current_set - e6_performers
    results.append(("E6: 3+ голосувань, не на 1 місці", [all_performers[pid] for pid in excluded_e6]))
    current_set = current_set - excluded_e6
    ...
    """)
    e7_performers = {row[0] for row in cursor.fetchall()}
    excluded_e7 = current_set - e7_performers
    results.append(("E7: Менше 2 балів", [all_performers[pid] for pid in excluded_e7]))
    current_set = current_set - excluded_e7

    # Фінальний список переможців
    winners = [all_performers[pid] for pid in current_set]
    
    return results, winners

def create_heuristics_plot(cursor):
    """Створює графік популярності евристик"""
    # Підрахунок використання кожної евристики
    heuristics_data = [
        ("E1: 1 раз на 3 місці", 0),
        ("E2: 1 раз на 2 місці", 0),
        ("E3: 1 раз на 1 місці", 0),
        ("E4: 2 рази на 3 місці", 0),
        ("E5: 1 раз на 3 і 1 раз на 2 місці", 0),
        ("E6: 3+ голосувань, не на 1 місці", 0),
        ("E7: Менше 2 балів", 0)
    ]
...
def vote():
    with sqlite3.connect("voting.db") as conn:
        ...
        if request.method == 'POST':
            ...
            # Перевірка на унікальність вибору
            if len(set([choice1, choice2, choice3])) != 3:
                return "Помилка: Ви не можете вибрати одного виконавця декілька разів", 400
            
            # Оновлюємо існуючий голос експерта
            cursor.execute("""
        ...
        # Підрахунок результатів з ваговими коефіцієнтами
        cursor.execute("""
            SELECT 
                p.name,
...
if __name__ == '__main__':
    ...
    # Змінюємо на звичайний запуск без debug режиму
    app.run()